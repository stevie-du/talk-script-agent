...
```bash
.venv\Scripts\python -m pytest        # 引擎侧 45 项（访问控制/状态机/校验器/模板/索引/建包/导出）
node _verify/verify.js                # 界面回归 50 项（桩 fetch，秒级）
node _verify/e2e-live.js              # 真实端到端（真引擎 + 真页面 + 真落盘）
```

三个都是零第三方依赖（除 pytest），可分别单独运行。
...
> 关掉之后，空态主区会显示配置引导（「先配置模型接口」+ 去配置按钮）。
>
> 没配过的字段在设置页里会标上「内置默认」小标 —— 引擎对缺失字段会取内置默认值，
> 不标的话「未配置」看起来和「已配置」一模一样（模型名、温度、超时全都像你自己填的）。
> 输入区的模型选择器同理，未配过时显示成 `glm-4.7（默认）`。
...
2. 默认**一键直通**；选**分步确认**则选题后暂停，确认/编辑角度再写文案
...
导出为 Agent 技能时 `private/` **默认不导出**（商业信息不外带），需要时显式开启。
