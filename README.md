...
单次生成的模型调用：**正常路径 2~3 次**（选题 1 + 撰写 1 + 回炉 0~2 轮，回炉轮是"撰写+校验"整体重跑）。⚠ 这是**无故障**的上界；加上请求层重试（`llm.retries`）、JSON 解析重试（`chat_json.max_retries`）、空内容预算升级重试，最坏单作业可达 32 个 HTTP 请求（见《审查报告-20260920》P1-5）。其余全是确定性代码。
