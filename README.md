...
分头落就会把红留给下一个人 —— 这段历史留在报告 §15.37 / §15.42，规矩留在这一节：

- **不要把这类断言改弱来"让门变绿"**。它红，说明有一半边没落；要找的是那半边，不是断言。
- 改这类断言时**同一笔提交**带上配套的 CSS（反之亦然），并在提交信息里写清它们是一对。
- 复量"提交态到底绿不绿"要用**干净检出**跑（`git worktree add` 到临时目录），不能拿工作树
  的结果代替 —— 工作树里带着未提交改动时看到的全绿，正是这类问题唯一的藏身处。
...
**导入第三方技能包**（2026-09-23 起，方案 `docs/技能包系统方案.md`）。
`%APPDATA%\TalkScript\packs\` 是**可写的包根目录** —— 除了 packgen 向导
生成的包，第三方技能包也导入到这里：

- **现在就能用**：把技能包目录（含 `pack.yaml` 的那层）整个拷进
  `%APPDATA%\TalkScript\packs\`，重启即可在行业选择里看到；
- 导入 API：`POST /api/packs/import`（zip 文件）与 `DELETE /api/packs/{name}`
  （卸载导入的包）。zip 会经过四道校验：条目白名单（**不含可执行文件**）、
  Zip Slip 逃逸、manifest/结构（引用的文件必须真实存在）、同名冲突
  （**你改过的内置包绝不静默覆盖**）—— 全部失败原因都是人话；
- 导入的包带 `.imported.json` 来源标记（作者/许可证/导入时间），界面上
  标「导入」；内置播种包不给卸载（删了下周播种又回来）；
- 第三方包的许可关系在**你与包作者**之间：本软件不预装、不推荐、不开市场，
  `license` 只在界面上原样展示。分享包 = 复制目录（不含 `private/`）。
- **给包作者**：制作指南与自检工具（`python -m app.packcheck <目录>`，
  判定与导入完全一致，退出码 0/1 可当打包门禁）见 `docs/技能包制作指南.md`。
...
4. 「换一版」是同一问题下的**新版本**，用消息上方的「1 / 2」翻页对照，不会覆盖上一版；
   而且换一版时模型**一遍思考出多版** —— 这一版里除主稿外还给至多 2 个不同表达的候选，
   用产物卡顶部的「主稿 / 候选 N」切换挑选。候选各自过合规校验，不合格的被拒并在
   作业日志里说明为什么少了一版（不是悄悄少给）
...
- [x] **A4** 定门槛：分布**已出**（`_verify/ai-smell-distribution.py`，
  ...
  **`list_enumeration` 阈值 3→4 已落码**（`ai_tells.py`，2026-09-23）：
  101 篇误伤清零（94 段→0）、满分 4→95、真 4 连排照抓，变异检验钉住
...
- [x] **B6** 选题视图 UI：左栏常驻入口 + 右栏切视图，不跳页、不整窗覆盖（`js/topics.js`）。
  ...
  `verify.js` 直接比它们的计算值
- [x] **B7** `select_ctx` / `draft_ctx` 注入 `$intel_block`（`app/intel.py::select_for_prompt`）：
  按 segment 挑最相关 3 条（本领域 > 通用 > 别的领域；有出处优先；机会分/新鲜度排序），
  **只给标题 + 出处（来源/日期/URL），不给原文** —— 待确认 #7：政策原文含 banwords hard 词，
  进正文层会被照抄后判违规。含 hard 禁用词的条目直接摘掉，**摘了的标题记进作业日志**（摘了要看得见）。
  两个包的 select / draft 模板**末尾**各接一处 `$intel_block`（`f418e79`）。
  - ⚠ **指纹的做法**（§2.9 B-3 落地）：`_plan_key` **只带"选中项 id + 出处"短键**（`intel_key`），
    情报块整块不进指纹。块在提示词末尾，按 `intel.PROMPT_HEAD` 首行标记切一刀。
    这样情报刷新、选中项没变 → 指纹不变 → 同一条选题仍可复现。
  - ✅ **守住了**：`tests/test_intel_b7.py` 7 条（反向断言模板真引用 `$intel_block` + 排序/摘词/不带原文 +
    指纹稳定性），4 道变异全红（摘引用 / 闸门失效 / 整块进指纹）。
