...
- ⚠ **安装流程本身还没在干净机器上验过**。安装包里的运行时能用已由「解包 + 端到端冒烟」
  ...
  3. 生成一个脚本 → 确认用的是**内嵌运行时**：日志里应出现 `engine/py（出厂运行时）`，
...
├── compliance/      # 广告法 / 平台差异 / 行业红线（红线经 write.files.redlines 注入）
...
- 撰写提示词里每轮真正变化的只有约 170 字，但回炉反馈块排在 user 模板**开头** ——
  上游的前缀缓存因此打不中（挪到末尾即可，见《审查报告-20260920》P1-40）
- 时长容差 `±max(10%, 3秒÷目标时长)` 在代码里算，包配置改不了（`checker.py` 的 `tolerance` 形参无人传）
...
现状：去AI味 100% 在提示词层（`voice` 三档 → `$anti_ai_rule` + `$voice_block`），
而 `check_script` 的 `passed` 只 gate 硬禁用词与时长偏差 —— **AI 味没有机器指标、不进回炉**，
所以"AI 味重但合规、时长准"的稿子一轮就过。

- [ ] **A1** `app/ai_tells.py`（纯函数无 IO）+ `packs/elevator/ai_tells.yaml`：
  每条 tell 带 `severity: strong|weak` + `check: 函数名` + `md: 清单锚点`；
  strong 单次命中即报、weak 需 ≥2 处。**词表只放词类**（书面连接词 / 抽象名词 / 口号 / 开场禁区），
  结构类（排比三连、段长等长、句首重复）留在函数里。
  ⚠ 复用 `checker.MIN_WORD_LEN` 那条下限，否则单字条目会把命中变成噪声
- [ ] **A2** `check_script` 的 report 加 `ai_smell: {score, hits[]}`，**先不进 `passed`**
- [ ] **A3** `tests/test_aitells_alignment.py`：yaml ↔ `patterns/anti-ai-smell.md` ↔ 函数名三方对账
  （形态照抄 `tests/test_banwords_alignment.py`）
- [ ] **A4** 校准后**才**定门槛：拿 `generated/` 现成产物跑分数分布，再决定 `ai_smell` 进不进回炉。
  顺序不能反 —— 未校准的 soft 词表曾 169/175 轮假命中
- [ ] **A5** 结果页 chips 加人味分 + `.banner.why` 折叠明细；`store.render_script_md` 同步一行
- [ ] **A6** 真人语料：`yt-dlp + faster-whisper` 出稿 → `packs/*/knowledge/samples/<style>/*.md`
  → `write_ctx` 注入 `$samples`（few-shot 的风格匹配远强于清单式提示词）。
  ⚠ 这是**离线工具**，不进分发链路；样本进提示词前要先过一遍 `banwords`，否则会连别人的毛病一起学
...
- [ ] **B2** **intel 落点改到 `data_dir`**：现在在仓库根，开发态能跑是因为 `data_dir` 默认等于 root，
  打包后安装目录不可写、会找不到（同 `store.py` 里 `generated/` 那条约定）
- [ ] **B3** `GET /api/intel/today`：只读本地文件，**空或坏返回 `[]` 不抛**
  （与 `private_facts()` 的失败语义**相反**，那边读不到要中止生成；这条差异要写进注释，
  否则将来一定有人"顺手统一")
- [ ] **B4** 懒触发：打开选题页时按"上次抓取距今"决定是否后台补抓，**走 Job 管道**
  （有状态/可取消/有日志；别学 `packgen` 那条同步 POST）
...
1. `ai_smell` 进不进回炉门槛（进 = 加轮次 = 加钱加时）
2. 左栏角标是「今日总数」还是「未读数」（未读要多存一个状态）
3. 「来源与抓取记录」面板做不做（不做的话重抓只能靠懒触发）
4. 真人语料谁挑：手工 30 条，还是批量 ASR 出 200 条再人工筛
