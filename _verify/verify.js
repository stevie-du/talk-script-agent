...
// ── 汇总输出 ────────────────────────────────────────────────
// 抽成函数是因为"分组早退"那条路径也要用它（早退时 results 里只有那一组）。
// ⚠ 那行「（分组：x）」标记不是装饰：mutate.py 靠它确认"确实只跑了这一组"。
//   没有它，一旦分组参数被无视而跑成整网，输出会是 353/357 —— 被读成
//   "分组报红"，而真相是压根没按分组跑。标记缺失 = 工具错，不是断言报红。
function report(onlyLabel) {
  console.log("\n════════ 界面回归结果 ════════");
  if (onlyLabel) console.log(`（分组：${onlyLabel}）`);
  let pass = 0;
...
check("取消编辑后回到当前启用的那条，且不留残余输入",
  ...
  await SECTIONS.topics({ evalIn, sleep, check });
