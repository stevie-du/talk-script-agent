...
class LLMClient:
    ...
    def _capped_read(base: float, deadline: float | None) -> float:
        ...
        那些检查点。最坏墙钟 = 结构层次数 × 请求层次数 × `timeout × STREAM_READ_TIMEOUT_MULT`
        （`chat_json` 的 `max_retries` 默认 1 → 2 次；请求层是 `retries + 1` 次；
        单次读超时 180s × 2 = 360s）：`retries=3` 时是 2×4×360 = 2880s ≈ 48 分钟，
        出厂默认 `retries=2` 时是 2×3×360 = 2160s = 36 分钟 —— 两种都是整作业预算
        （1200s）的 1.8 ~ 2.4 倍，槽被一个卡死的上游占着。
        写清"在哪个配置下算出来的"是因为 `retries` 用户可配（校验区间 0~10），
        只留一个不带口径的数就会过期（本项目反复出过这种账）。
