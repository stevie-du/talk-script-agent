...
// ⚠ 必须写转义序列 "\u200B"，**不许**把不可见字符直接粘进源码 —— 本文件曾经
// 就是这么坏的（2026-09-26 P2-9，字节级核实引号内零字节）：某次保存把那个
// 不可见字面量剥掉了，ZWSP 变成空串，两次 toast 产出完全相同的文本，
// aria-live 的重复播报机制整体静默死亡。转义序列是 ASCII 源码，任何编辑器
// 都剥不掉它；这个教训适用于一切「源码里需要不可见字符」的场合。
const ZWSP = "\u200B";
